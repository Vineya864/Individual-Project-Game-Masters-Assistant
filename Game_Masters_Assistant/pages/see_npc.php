...
/**
 * see all npcs 
 * manage active npcs and get npc profiles create ne npc
 */
?>
<head>
	<meta charset ="UTF-8"/>
...
$role	       = $_SESSION["role"];
$npc_model     = new NPCCharacterController(null,null);
$campaign_model= new CampaignController(null,null);
if (isset($_POST["Open_Details_NPC"])) {
	if (hash_equals($_SESSION['token'], $_POST['token'])) {
	$_SESSION['current_npc']= $_POST['user_character'];
	?>
	<script> window.open("npc_details.php");
	window.location.replace("see_npc.php"); </script>
	<?php
	}else{
		tokenError();
	}
...
if (isset($_POST["npc_creation"])) {
	...
	?>
	<script>
	 window.location.replace("npc_creation.php"); 
	</script>
	<?php
	}else{
		tokenError();
...
if (isset($_POST["make_active"])) {
	if (hash_equals($_SESSION['token'], $_POST['token'])) {
		...
		<script>window.location.replace("see_npc.php"); </script>
	<?php
		}else{
			$url='login.php';
			echo '<META HTTP-EQUIV=REFRESH CONTENT="1; '.$url.'">'; 
...
<form action="" method="post" >
	...
	<input type="submit" value="Create New NPC" name= "npc_creation">
</form>
</div>
<section>
...
<div id="all_npc">
<?php
$npc_model->showCampainCharactersActiveStatus($campaign_id,$token)
